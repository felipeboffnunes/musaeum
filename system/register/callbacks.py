...
from system.layouts.fragments.menu import get_sidebar
...
def register_callbacks(app):
    ...
    def render_page_content(pathname):
        if pathname == "/":
            ...
            content = html.Div([pages["ABT"]], id="page-content")
            return get_sidebar(collapse=True), content

        return get_sidebar(), html.Div([dbc.Jumbotron(
            [
    ...
    def update_library_modal(active_cell, data):
        ...
        if active_cell["column"] == 1:
            return [True, authors[data[active_cell["row"]]["Author"]]["description"],
                    data[active_cell["row"]]["Author"]]
        return [dash.no_update, dash.no_update, dash.no_update]
